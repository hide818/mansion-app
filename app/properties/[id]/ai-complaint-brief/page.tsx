...
export default async function AiComplaintBriefPage({ params }: PageProps) {
  ...
  const contextText = `【AIへ渡すクレーム共有用データ】
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-wrap gap-3">
        ...
        <Link href={`/complaints`} className="px-4 py-2 rounded-lg border bg-white hover:bg-gray-50">
          クレーム一覧へ
        </Link>
      </div>

      <div>
        <h1 className="text-2xl font-bold">AIクレーム要約（共有向け版）</h1>
        <p className="text-sm text-gray-600 mt-2">
          物件カルテや引き継ぎ時に貼りやすい、クレーム共有用の要約文をAIで作ります。
        </p>
      </div>
      ...
      <AiTextGenerator
        title="AIクレーム要約（共有向け）"
        description="傾向、注意点、共有すべきことを短めに整理して生成します。"
        apiPath={`/api/properties/${propertyId}/ai-complaint-brief`}
      />
    </div>
  )
}