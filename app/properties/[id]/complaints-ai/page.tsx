...
export default async function ComplaintsAiPage({ params }: PageProps) {
  ...
  const contextText = `【AIへ渡すクレームデータ】
...
${
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold">AIクレーム要約</h1>
        <p className="text-sm text-gray-600 mt-2">
          クレームの傾向、再発しやすいテーマ、社内共有用の要約文をAIで作ります。
        </p>
      </div>
      ...
      <AiTextGenerator
        title="AIクレーム要約"
        description="クレームの傾向と再発防止コメントを含めた要約文を生成します。"
        apiPath={`/api/properties/${propertyId}/ai-complaint-summary`}
      />
    </div>
  )
}