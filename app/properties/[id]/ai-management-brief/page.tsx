...
export default async function AiManagementBriefPage({ params }: PageProps) {
  ...
  const contextText = `【AIへ渡す管理ブリーフ元データ】
...
${
  safeCases.length === 0
    ...
    : safeCases
        .map(
          (item) =>
            `・${item.title ?? '案件名未設定'} / 状況:${item.status ?? '未設定'} / 担当:${item.assignee ?? '未設定'} / 理事会:${item.board_status ?? '未設定'} / 上程予定:${formatDate(item.board_scheduled_for)}`
        )
        .join('\n')
}
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-wrap gap-3">
        ...
        <Link href={`/properties/${propertyId}/tools`} className="px-4 py-2 rounded-lg border bg-white hover:bg-gray-50">
          物件ツール一覧へ
        </Link>
      </div>

      <div>
        <h1 className="text-2xl font-bold">AI対応提案（物件管理ブリーフ）</h1>
        <p className="text-sm text-gray-600 mt-2">
          物件全体の案件・タスク・クレームを見て、管理担当として今優先すべきことをAIが整理します。
        </p>
      </div>
      ...
      <AiTextGenerator
        title="AI対応提案（物件単位）"
        description="優先順位つきで、今やるべきこと・今週やること・後回しにできることを生成します。"
        apiPath={`/api/properties/${propertyId}/ai-management-brief`}
      />
    </div>
  )
}