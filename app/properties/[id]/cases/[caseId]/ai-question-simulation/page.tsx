...
export default async function AiQuestionSimulationPage({ params }: PageProps) {
  ...
  const { data: caseItem } = await supabase
    .from('cases')
    .select(
      'id, title, status, assignee, board_status, board_scheduled_for, board_agenda_title, board_decision_note, board_next_action'
    )
    .eq('id', caseId)
  ...
  const contextText = `【AIへ渡す理事会シミュレーション元データ】
    ...
    <div className="p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-600 mt-2">
          理事会での質問と受け答えを、会話形式で先にシミュレーションします。
        </p>
      </div>
      ...
      <AiTextGenerator
        title="AI理事会シミュレーション"
        description="理事長・役員からの質問と、管理会社としての回答例を会話形式で生成します。"
        apiPath={`/api/properties/${propertyId}/cases/${caseId}/ai-question-simulation`}
      />
    </div>
  )
}