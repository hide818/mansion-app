...
export default async function AiPropertyNextActionsPage({ params }: PageProps) {
  ...
  const contextText = `【AIへ渡す物件単位の判断材料】
...
${
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold">物件単位のAI次アクション提案</h1>
        <p className="text-sm text-gray-600 mt-2">
          物件全体で今なにを優先すべきか、案件・タスク・クレームを横断してAIが提案します。
        </p>
      </div>
      ...
      <AiTextGenerator
        title="物件AI次アクション提案"
        description="物件全体で優先すべき動きを、順番つきで生成します。"
        apiPath={`/api/properties/${propertyId}/ai-property-next-actions`}
      />
    </div>
  )
}