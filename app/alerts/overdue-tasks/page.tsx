...
export default async function OverdueTasksPage() {
  ...
  if (error) {
    console.error('overdue tasks page error:', error)

    return (
      <div className="p-6">
        <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">
          期限切れタスクの取得に失敗しました。tasks テーブルの company_id と due_date を確認してください。
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">期限切れタスク一覧</h1>
        <p className="mt-2 text-sm text-gray-600">
          未完了かつ期限を過ぎているタスクを、期限が古い順で表示しています。
        </p>
      </div>

      <div className="mb-6 grid gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-red-200 bg-red-50 p-4 shadow-sm">
          <div className="text-sm text-red-700">期限切れ件数</div>
          <div className="mt-2 text-3xl font-bold text-red-900">{safeTasks.length}</div>
        </div>

        <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
          <div className="text-sm text-gray-500">対象</div>
          <div className="mt-2 text-sm font-medium text-gray-900">未完了タスクのみ</div>
        </div>

        <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
          <div className="text-sm text-gray-500">並び順</div>
          <div className="mt-2 text-sm font-medium text-gray-900">期限が古い順</div>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        ...
        {safeTasks.length === 0 ? (
          <div className="px-4 py-6 text-sm text-gray-500">期限切れタスクはありません。</div>
        ) : (
