...
export async function GET(_request: NextRequest, context: RouteContext) {
  try {
    ...
    if (propertyError) {
      return NextResponse.json(
        { error: '物件名の取得に失敗しました。', detail: propertyError.message },
        { status: 500 },
      )
    }
    ...
    const { data: linkedCasesData, error: linkedCasesError } = await supabase
      .from('cases')
      .select('id, title, status, created_at')
      .eq('company_id', companyId)
      .eq('source_minutes_record_id', recordId)
      .order('created_at', { ascending: false })

    if (linkedCasesError) {
      return NextResponse.json(
        { error: '関連案件の取得に失敗しました。', detail: linkedCasesError.message },
        { status: 500 },
      )
    }
    ...
    if (linkedTasksError) {
      return NextResponse.json(
        { error: '関連タスクの取得に失敗しました。', detail: linkedTasksError.message },
        { status: 500 },
      )
    }

    let sourceRecordTitle = ''
    ...
    if (row.source_record_id) {
      ...
      sourceRecordTitle = sourceData?.title ?? ''
    }
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (targetError) {
      return NextResponse.json(
        { error: '更新対象の確認に失敗しました。', detail: targetError.message },
        { status: 500 },
      )
    }
    ...
    if (error) {
      return NextResponse.json(
        { error: '議事録の上書き保存に失敗しました。', detail: error.message },
        { status: 500 },
      )
    }
...
export async function DELETE(_request: NextRequest, context: RouteContext) {
  try {
    ...
    if (error) {
      return NextResponse.json(
        { error: '保存済み議事録の削除に失敗しました。', detail: error.message },
        { status: 500 },
      )
    }
